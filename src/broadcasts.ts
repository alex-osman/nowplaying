...
export const vote = (post: Post, votingPower: number = 1) => {
    return new Promise((resolve, reject) => {
        steem.broadcast.vote(wif, username, post.author, post.permlink, votingPower * 100, (err, result) => {
            if (err) {
                if (err.data.code === 10) {
...
export const comment = (post: Post, body: string) => {
    return new Promise((resolve, reject) => {
